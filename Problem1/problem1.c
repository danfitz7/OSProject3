...
thread_id get_next_thread_to_run(){
	...
	*/
	...
	if (TS_queue.count >=3 || pending_TS==True){
		...
		if (cur_queue_index == U){
			pending_switchType = TS;//cur_queue_index = TS;
		}else{
			cur_queue_index = TS;
		}
		//return pop_queue(&TS_queue);
	}else if (cur_queue_index == S || cur_queue_index == TS){
		if(S_queue.count>0){
			cur_queue_index = S;
			...
			if (U_queue.count > 0){
				pending_switchType = U;//cur_queue_index = U;
			}else{
				printf("WARNING: Queues empty!");
				while(U_queue.count > 0 && S_queue.count > 0 && TS_queue.count > 0);
				if (U_queue.count > 0 ){
		...
		if (U_queue.count > 0){
			cur_queue_index = U;//cur_queue_index = U;
		}else{
			printf("WARNING: Queues empty!");
			while(U_queue.count > 0 && S_queue.count > 0 && TS_queue.count > 0);
			if (U_queue.count > 0 ){
	...
	if (pending_switchType== IDLE && !pending_TS){
